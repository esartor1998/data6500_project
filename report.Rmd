...
### Preliminary Work Results
  ...
  While the resultant model itself is the key focus of this report, the preliminary work also yielded some interesting results worthy of discussion. The focal correlation plot generated for Figure (???) is primarily reassuring due to the large swathe of reasssuringly negatively correlated space, however the areas which are positively correlated are also interesting and point to additional conclusions that can be gleaned from the data. Major cities of such as Paris, Berlin, and Vienna are counterintuitively strongly _positively_ correlated with population difference. In other words: despite being dense urban areas, they seemed to experience relative growth in contrast to other major cities of the time which were strongly negatively correlated, like London, Munich, or Milan. This could point to a pattern of migration into urban areas from less-affected rural communities as a result of the severe and abrupt population shock [@DorseyArmstrongHowPolandMilan2021; @Siuda.SundeDiseaseDemographicDevelopment2021; @Jedwab.etalPandemicsPlacesPopulations2019]. This phenomenon is of immense interest to anthropologists, especially those interested in human movement and the dramatic societal and cultural shifts that manifested in the regions where this positive correlation is present shortly after the most devastating years of plague had passed @A.D.A.M.Inc.Plague. Unfortunately, the model utilized in this paper is not robust enough to make predictions about this phenomenon, though it presents a promising avenue for future work. 


```{r focal_correlation_between_deaths_and_popd, echo=FALSE, fig.cap='Pop. Dens. and Pop. Diff. Spatial Focal Correlation'}
...
```{r predicted_intensity_through_rhohat, echo=FALSE, fig.cap='Nonparametric Estimate of Intensity as a Function of Pop. Dens.'}
...
d_smoothing = c(0.2, 0.6, 0.8) #size of the region extremely close to 1.0 in distance raster
...
params = crossing(d_smoothing, d_coeff, d_max_range, worst_case, mortality)
iter=0
susc_maps = list()
final_dead = list()
final_alive = list()
...
- areas of population growth not accounted for.
- balkans not accurate at all for some reason; some other factor?

- does not account for saturation of popd. as a proxy for contacts (heat equation in that papper)


### Deterministic Model Results

  Despite its relative simplicity, the deterministic model performs extremely well on this data. With a very strong correlation coefficient between the model-estimated population and that estimated by the HYDE, we can be fairly confident that at least against the HYDE 3.2 data, predictions are fairly accurate. However, the system's overreliance on self-verification using those estimates is also a significant weakness of the system; in the future, it would be prudent to test against other estimates of population and population decline, even simply testing the population of cities against what is reported by other historical sources. Furthermore, it would be prudent to test far more parameter sets than are tested here, however due to the exponentially-increasing compute time that grid search consumes as parameters are added, in the interest of time, the current parameter set provides a reasonable enough fit.
  
  Of the parameters that are present, some seem to be far more influential than others. For example, 
  
  Additionally, it is interesting that the preferred model differs between population count and deaths. This may also be the result of a dearth of adequate parameters in the system. Because deaths and surviving population are two sides of the same coin, linked mathematically through the system $A = R + S, R = I - D, D = mortality \times I$, it seems fairly intuitive that adding at the very least a hyperparameter for $mortality$ instead of having it as a constant value would likely help to remedy this issue. Furthermore, some plague models [@Dean.etalHumanEctoparasitesSpread2018; @Sichone.etalEstimatingBasicReproduction2020; Nguyen.etal2017PlagueOutbreak2018] add differing mortality and infection rates for various forms of the disease, as well as the capability to transition between these states. This certainly would likely also be a key step towards more accurate and realistic modelling. Unfortunately, doing so would preclude the continued usage of grid search to tune the parameters, as it would necessitate the addition of a huge swathe of new hyperparameters, likely skyrocketing the required computational time for this method far, far into the realm of infeasability.
  
  As population increases, both correlation plots seem to diverge into two pathways, one in which my estimates are too high, and one in which they are too low. This seems to indicate that the current set of parameters this system applies to the data is not adequate, and that there are other factors which are unaccounted for. @Hu.etalScalingContactRates2013
  
  

- note the implications behind the parameters which were successful, esp. that worst_case.
- note that some parameters are useless and still more parameters are likely necessary to get higher accuracy


```{r top_correlation_chart_A, echo=FALSE, out.height='80%', fig.cap='Most Correlated Population Model Estimated vs. Actual Plot'}
...
```{r best_model_result_A, echo=FALSE, fig.cap='Comparison of 1400AD Population Estimate Rasters', fig.height = 27, fig.width = 20}
...
```{r top_correlation_chart_D, echo=FALSE, fig.cap='Most Correlated Death Model Estimated vs. Actual Plot', out.height='80%'}
...
```{r best_model_result_D, echo=FALSE, fig.cap='Comparison of 1400AD Population Estimates', fig.height = 27, fig.width = 20}
