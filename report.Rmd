...
#this needed some tinkering in order to get looking decent, and will likely need more
#tinkering in order to create a sensible mode
distance = list()
i = 0 #i see no need to use any fancy tricks here, when an iterating variable will work
anim_coll = lapply(recorded_years, function(year) { #for all years
  i = i + 1
  prev_years = target_years[which(target_years$year <= year),]
	curr_year = target_years[which(target_years$year == year),]
	
	dist_rast = exp(1)^(-((terra::distance(ad1300_popc_rast, vect(curr_year), rasterize=T)^(2/3))/2e3))
	raw_inf_rast = (dist_rast * sqrt(ad1300_popd_rast) * r0)
	
	print(tm_shape(basemap) + tm_rgb(alpha=0.7) + tm_shape(raw_inf_rast, raster.downsample=F) + tm_raster(title='Raw Infectivity',legend.show=T) +
	  tm_shape(maritime_routes_sf) + tm_lines(col='dodgerblue', alpha=0.5) +
    tm_shape(borders$geometry) + tm_borders(alpha=0.5) + 
		tm_shape(prev_years) + tm_dots(col='gray48') +
	  tm_shape(curr_year) + tm_dots(col='is_reoccurrence', palette=c('red','blue'), legend.show=FALSE) +
	  tm_add_legend(type=c('symbol'), labels=c("New Outbreak", 'Recurring Outbreak', 'Previous Outbreak'), col=c('red', 'blue', 'gray48')) +
    tm_layout(legend.position=c("left", "top"), frame=FALSE, legend.bg.alpha=0.6, main.title=year))
})
