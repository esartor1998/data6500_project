...
#so, the r_prev doesn't matter, because yearly recurring outbreaks are recorded
#but popd/popc decreases can be used to impl. a -temporal effect on r_t instead
...
for (year in recorded_years) {
  ...
  prev_years_outbrks = target_years[which(target_years$year <= year),]
	...
	#the distance component of r_t
	dist_rast = 
	  exp(1)^(-((terra::distance(ad1300_popc_rast, vect(curr_year_outbrks), rasterize=T)^(2/3))/2e3))

	#THE MOST IMPORTANT LINE OF CODE:
	infn_rasters[[curr_idx]] = (dist_rast * sqrt(ad1300_popd_rast) * r0)
	prev_infn = ifelse(prev_idx > 0, 
	                  infn_rasters[prev_idx],
                    1)
	
	maps[[curr_idx]] = 
	  tm_shape(basemap) +
	    tm_rgb(alpha=0.7) +
	  tm_shape(infn_rasters[[curr_idx]], raster.downsample=F) +
