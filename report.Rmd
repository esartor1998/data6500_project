...
It is crucial to note that $dens_{rast}$ is not normalized, thus cells with a high density of susceptible individuals will be struck most severely with plague. However, because the susceptible population density is adjusted each iteration, this is seen most acutely when the area experiences its first outbreak. Additionally of note is that the value of $dist_{rast}$ approaches 0 at an exponential rate with distance to a recorded outbreak, allowing the progression of the disease to roughly follow the point pattern present in the outbreaks dataset, and for the countryside to be less severely affected than denser, more urban areas where outbreaks are typically recorded.
  ...
  Following this, the newly modified $S$ is utilized to create a new $dens_{rast}$ for the next iteration. However, this is not straightforward: even though the data is unprojected, the ellipsoid indexed by the longitude and latitude coordinates is not perfectly spherical and is instead flatter about the poles. Thus, some cells represent a larger actual area than others, and the true geographic area represented needs to be calculated so that the population density is not skewed positively towards flattened areas. Luckily, `terra`'s `cellSize` is a quick and effective way to obtain the true geographic size of a raster cell, provided that it is sufficiently small. However, this alone still does not perfectly convert population to population density, as the HYDE uses other sources in addition to population count to gauge population density. As a result, we also need to offset each population count-to-density conversion by the difference between 1300AD population density calculated using only 1300AD population count, and the 1300AD HYDE estimate for population density. Put mathematically, the offset raster is calculated through $b = \frac{count_0}{size_0} - dens_0$, where $count_0$ is the population count raster in 1300AD, $size_0$ represents the raster of cell sizes obtained with `terra` for all cells in that raster, and $dens_0$ represents the population density estimate for 1300AD. This offset raster only needs to be calculated one time, before recursion has begun, as it acts as a global adjustment to the values calculated. Further, the function used to convert population count to population density could be expressed as $\frac{count}{size}-b$, where $count$ is the population count raster, $size$ is the calculated raster containing $counts$ cell sizes, and $b$ is the previously-calculated offset.
  
  With all this complete, $I$ is multiplied by $mortality$ in order to approximate the number of those infected who will die; for this simulation, a scalar $0.8 = mortality$. This 80% mortality rate approximation is based upon various estimates of the average contemporary mortality across all forms of the disease [@Pechous.etalPneumonicPlagueDarker2016; @EvansPneumonicPlagueIncidence2022; @Dean.etalHumanEctoparasitesSpread2018; @TotaroSufferingParadiseBubonic2005; @CohnEpidemiologyBlackDeath2008]. The resulting raster, $D$, represents only the dead individuals from the outbreak year currently analysed, not the cumulative number of deaths over the entire epidemic. All yearly $D$ rasters must be summed at the end of recursion in order to compute a total estimate.

  The algorithm is run until final desired year is reached. At that point, the $D$ rasters are all summed together, generating an estimate for the total number of deaths per cell over the 14th century. The correlation between this approximation and the inverse of the death raster (as the values computed by the model are positive, while cumulative deaths are a negative number on the population difference raster) is also computed. The correlation between the final susceptible population estimate $S$ and the population count in 1400AD is also tracked. The model is then run several more times, with differing $m$, $a$, $x$, and $s$ parameters, and the most correlated models are selected and displayed, visible in Figure (???), and in Figure (???).
...
d_smoothing = c(0.2, 0.8)
              #seq(0.20, 0.80, by=0.2) #size of the region extremely close to 1.0 in distance raster
d_coeff =     c(2, 10)
              #seq(2, 10, by=2) #size of the overall region above d_threshold in distance raster
d_max_range = c(3, 12)

worst_case =  c(0.75, 0.9)
              #seq(0.3, 0.9, by=0.3) #estimated maximum yearly saturation % of infection among S
mortality   = c(0.6, 0.8) #@TotaroSufferingParadiseBubonic2005
...
- d_coeff does not matter at all lawl

```{r top_correlation_chart, echo=FALSE}
cord2=list()
for(i in 1:length(final_dead)) {
  cord2[[i]] = cor(terra::values(-death_raster),
                   terra::values(final_dead[[i]]), 
                   use="complete.obs")
}

mc2 = which.max(cord2)
plot(terra::values(final_dead[[mc2]]) ~ terra::values(-death_raster),
     xlab = 'Actual Population Decrease',
     ylab = 'Expected Population Decrease')
abline(lm(terra::values(final_dead[[mc2]]) ~ terra::values(-death_raster)))
legend("topleft", legend=paste0('Model #', mc2,
                               ' Final Death Correlation = ', round(cord2[[mc2]], 2)))
...
most_corr_idx_P = which.max(correlations_P)
plot(terra::values(final_susc[[most_corr_idx_P]]) ~ terra::values(ad1400_popc_rast),
     xlab = 'Actual Final Population',
     ylab = 'Expected Final Population')
abline(lm(terra::values(final_susc[[most_corr_idx_P]]) ~ terra::values(ad1400_popc_rast)))
legend("topleft", legend=paste0('Model #', most_corr_idx_P,
                               ' Final Pop. Correlation = ', round(correlations_P[[most_corr_idx_P]], 2)))
```
...
tm_shape(basemap) +
  tm_rgb(alpha=0.8) +
  tm_shape(final_susc[[most_corr_idx_P]], raster.downsample=F) +
  tm_raster(alpha=0.7, 
            breaks=c(0.0,
                     100.0,
                     200.0,
  ...
  tm_layout(legend.position=c('left', 'top'),
            frame=FALSE,
            legend.outside=TRUE,
            main.title=paste0('Model #',most_corr_idx_P,' Predicted Population in 1400AD'))
```
