...
unproj_bbox = st_bbox(c(xmin = -16.26,
                 xmax = 42.98,
                 ymin = 27.92,
                 ymax = 61.54),
                 crs=unprojected_crs) %>%
...
d_smoothing = c(0.3)#, 0.6, 0.9) #size of the region extremely close to 1.0 in distance raster
                               #girls wanna drink wiyne
...
correlations_D = list()
correlations_P = list()
for (trial in split(params, seq(nrow(params)))) { #this could be made far more efficient, but no time
  iter = iter + 1
  susceptible = list()
  infected = list()
  recovered = list()
