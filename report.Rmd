...
target_years = all_target_years[which(all_target_years$year > 1345 & all_target_years$year <= 1400),]
recorded_years = unique(target_years$year)
first_year = recorded_years[1]

#extremely rough, 'death guesstimation' model:
#d_smoothing affects the size of the region extremely close to 1.0 in distance raster
#d_coeff     affects the size of the overall region above d_threshold in distance raster
#d_threshold is the value at which we would like to round to 0, to prevent global infection
d_smoothing = seq(0.30, 0.70, by=0.2)
d_coeff =     seq(4, 8, by=2)
#d_threshold will not be tested, as it's just to ensure sensible results
#estimated maximum yearly saturation % of infection among susceptible:
worst_case =  seq(0.2, 0.8, by=0.3)

#non-tunable params
d_threshold = 0.01
mortality   = 0.8

params = crossing(d_smoothing, d_coeff, worst_case)
iter=0
susc_maps = list()
final_susc = list()
for (trial in split(params, seq(nrow(params)))) {
  ...
  for (year in recorded_years) {
    ...
    if(year == 1400) {
      final_susc[[iter]] = susceptible[[curr_idx]]
      susc_maps[[iter]] = 
        tm_shape(basemap) +
        tm_rgb(alpha=0.8) +
        ...
        tm_raster(alpha=0.7, 
                  breaks=c(0.0,
                           ...
                           24000.0,
                           48000.0),
                           title=paste('Model',iter,'1400AD Susceptible Count'),legend.show=T)
    }
  }
  print(trial)
...
for (i in 1:length(susc_maps)) {
  plot(terra::values(final_susc[[i]]) ~ terra::values(ad1400_popc_rast))
  abline(lm(terra::values(final_susc[[i]]) ~ terra::values(ad1400_popc_rast)))
  legend("topleft", legend=paste('Model', i,
                                 'Correlation =', round(cor(terra::values(ad1400_popc_rast),
                                                            terra::values(final_susc[[i]]), 
                                                            use="complete.obs"), 2)))
  print(susc_maps[[i]])
}

# print(susc_maps[[year2idx(1400)]])
# 
# tm_shape(basemap) +
#   tm_rgb(alpha=0.8) +
...
#                      24000.0,
#                      48000.0),
#                      title='HYDE 3.2 1400AD Pop. Count',legend.show=T)
