...
for (year in recorded_years) {
  ...
  prev_rt = ifelse(prev_idx > 0,
                   rt_rasters[prev_idx],
                   1
            ) #if this doesnt work irght im gonna kms
  print('curr')
  print(curr_idx)
  print('prev_rt')
  print(prev_rt)
	...
	raw_inf_rast = (rt_rasters[[curr_idx]] * sqrt(ad1300_popd_rast) * r0)
	
	maps[[curr_idx]] = 
	  tm_shape(basemap) +
	    tm_rgb(alpha=0.7) +
	  tm_shape(raw_inf_rast, raster.downsample=F) +
