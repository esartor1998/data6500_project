...
#not entirely sure why this data got put into hectares by default but whatever

popc2popd = function(popc_rast) {
  raw_conv_raster = popc_rast / popc_cell_size
  return(raw_conv_raster - (raw_conv_raster - ad1300_popd_rast))
}

#error checking this...
tm_shape(popc2popd(ad1300_popc_rast) - ad1300_popd_rast) +
  tm_raster(n=30, title='Diff. in converted popd from actual in 1300')
#very consistent~
#there are some very slight discrepancies where other sources have been used than just population
#count in order to estimate density in HYDE, but really it's pretty decently accurate
#so, in order to account for these discrepancies, we can just add them in the conversion :)
...
r0 = 1.765 #this is the unscaled, un-time-based value. also, its per-diem, so... (cite this moron)
...
for (year in recorded_years) {
  ...
  prev_years_outbrks = target_years[which(target_years$year <= year),]
	...
	susd_tmp = if(length(susd) == 0) ad1300_popd_rast else susd[[prev_idx]]
	
	#the distance component of r_t. exponential decay based on (scaled) distance
	dist_rast =
	  exp(1)^(-((terra::distance(susc_tmp, vect(curr_year_outbrks), rasterize=T)^(2/3))/2e3))
	...
	#simple model. density is exponential, which is good for us
	infected[[curr_idx]] = susd_tmp * infn_raster * 36.5
	...
	susd[[curr_idx]] = susceptible[[curr_idx]] / 4 #tmp
