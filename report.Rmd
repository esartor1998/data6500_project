...
#model values for the calculation of β per cell: 
dist_smoothing = 0.4       #affects the size of the region extremely close to 1.0 in distance raster 
dist_coefficient = 6       #affects the size of the overall region above 0.0 in distance raster
dist_threshold = 0.00000001#value at which we would like to round to 0, to prevent global infection

r0 = 1.4 #using median of several estimates. this is per inf. period, so we need to scale for time
r0 = r0 * 36.5 #so very oversimplified
#population density is a proxy for number of contacts/time in addition to whats included in r_0
popd_maxadjust = 15.0 * 36.5
#popd_maxadjust is the maximum additional number of contacts per time that popd can factor in
#"transmission probability" we will say scales with distance from an outbreak for this model,
#as well as the proportion of individuals in a given cell that are infected

mortality = 0.8 #cite the papers that you inferred/calc'd this from
...
#this model does not use this yet, but a second, more complicated model will
...
for (year in recorded_years) {
  ...
  if (length(susceptible) != 0) {
    plot(cumsum(recovered), main=paste('cumsum recovered', year))
  }
  
  susd_tmp = popd_maxadjust * normalize_rast(
    if(length(susd) == 0) ad1300_popd_rast else susd[[prev_idx]]
  )
  
  #the distance component of r_t. exponential decay based on (scaled) distance. scaled, tapered, and
  #clamped so that it is sane and reaches 0 at a "reasonable" distance.
  #the 10 is so that the values e is raised to are between 0 and 10 (fractional exponents are weird)
  dist_rast = exp(1)^(-10 * normalize_rast(
    dist_coefficient*terra::distance(susc_tmp, vect(curr_year_outbrks))^dist_smoothing
  ))
  
  dist_rast[dist_rast < distance_threshold] = 0
  plot(susd_tmp, main=paste('susd', year))
  #the "infectiousness raster" we can use as a term in our estimate of the number of dead/infected
  #this represents the scaling of the infectiousness over distance.
  infn_raster = (dist_rast * r0)
  plot(infn_raster, main=paste('infn_rast', year))
  
  #the estimate of the infected. should i use terra::predict, or...?
  infected[[curr_idx]] = floor(susd_tmp * infn_raster) #going to rework this into proper SIR
  ...
  plot(susd[[curr_idx]], main=paste('susc. dens.', year)) #why is it always the same?
  
  maps[[curr_idx]] = 
    tm_shape(basemap) +
      tm_rgb(alpha=0.7) +
    tm_shape(infected[[curr_idx]], raster.downsample=F) +
      tm_raster(title='Estimated Infected',
                legend.show=T,
                breaks=c(0,
                         1,
                         50,
...
#for (i in 1:length(susd)) {
#  print(tm_shape(susd[[i]]) + 
#          tm_raster())
#}
