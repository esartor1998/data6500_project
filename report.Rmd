...
notes: please change the simulation back to normal when done
...
Unless otherwise specified, all analyses were performed using the R language @rcoreteamLanguageEnvironmentStatistical2023. Firstly, a raster containing the population difference between 1300AD and 1400AD was created. This raster is variously referred to in this report as the "death raster", or "population difference raster". It represents the loss or gain in population over the 14th century as estimated by the HYDE. In a normal century, it would be expected that the population would grow in most regions, but over the course of the 14th century there was significant population decrease in most regions. As a result, and given the general small magnitude of such positive numbersFocal correlation was assessed using the appropriate function through `terra`'s `focalPairs` function @hijmansTerraSpatialData2023.
- the plague over death raster
- the correlation between population density and deaths, and WHY its important/'_surprising_'
...
```{r focal_correlation_between_deaths_and_popd}
...
```{r predicted_intensity_through_rhohat}
...
tm_shape(basemap) + tm_rgb(alpha=0.8) + tm_shape(predrast, raster.downsample=FALSE) +
                                        tm_raster(alpha=0.7,
                                                  ...
                                                  title = 'Intensity') +
  tm_shape(borders$geometry) + tm_borders(alpha=0.5) + 
  tm_layout(legend.position = c('left', 'top'), main.title = '14th Century Predicted Intensity')
```
...
tm_shape(basemap) +
  ...
  tm_raster(alpha=0.7, 
            breaks=c(0.0,
                     ...
                     24000.0,
                     48000.0),
                     title=paste('Model',most_corr_idx,'Cumulative Dead'),legend.show=T)
```
