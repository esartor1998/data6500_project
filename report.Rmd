...
worst_case = 0.6 #assume that at most 20% of a cell can be I in any given outbreak yr (armstrong)
...
for (year in recorded_years) {
  ...
  dist_rast = exp(1)^(-3 * normalize_rast(
    dist_coefficient*terra::distance(susc_tmp, vect(curr_year_outbrks), rasterize=T)^dist_smoothing
  ))
  ...
  infn_rast = worst_case * dist_rast * susd_tmp
  ...
  susc_maps[[curr_idx]] = 
    ...
    tm_raster(alpha=0.7, 
              breaks=c(0.0,
                       ...
                       24000.0,
                       48000.0),
                       title='Modelled 1400AD Pop. Count',legend.show=T)
}
